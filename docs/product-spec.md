...
The initial slice supports seeded development organizations/users, development persona authentication, case creation/list/details/status changes, assignment on creation when the user belongs to the organization, idempotency, optimistic concurrency, immutable creation audit records, transactional outbox records, worker processing, health endpoints, and telemetry. See README for the exact status.
